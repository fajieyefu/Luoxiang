...
public class InvoiceManageActivity extends BaseActivity implements XListView.IXListViewListener, View.OnClickListener {
    @BindView(R.id.title)
    TitleLayout title;
    ...
    private void initView() {
        ...
        historyLv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                intent.setClass(InvoiceManageActivity.this, InvoiceDetailActivity.class);
                intent.putExtra("orderId", orderId);
                intent.putExtra("orderNumber",contracts.get(position-1).getOrderNumber());
...
//        loadData();
    ...
    private class MyCallBack extends MyCallback2 {
        ...
        public void onError(Call call, Exception e, int id) {
            Toast.makeText(InvoiceManageActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
        }
        ...
        public void onResponse(ResponseBean2 response, int id) {
            ...
            if (response.getCode() == 0) {
                if (response.getData() != null) {
                    List<ContractBean> datas = response.getData();
                    for (ContractBean contractBean:datas){
                        if (contractBean.getNq_flag()!=1){
                            continue;
                        }
                        contracts.add(contractBean);
                        contractResult.add(contractBean);
                    }
                    chooseType.setText("全部  ");
                }
            } else {
                Toast.makeText(InvoiceManageActivity.this, response.getMsg(), Toast.LENGTH_SHORT).show();
            }
            if (contracts.size() == 0) {
